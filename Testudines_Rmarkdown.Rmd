...
I collected turtle life-history data by downloading the Amniote_Database_Aug_2015.csv life-history [database](http://www.esapubs.org/archive/ecol/E096/269/#data). The amniote dataframe by Myhrvold et al.(2015) included 21,322 objects with 36 variables.  I analyzed these data with RStudio using the tidyverse package. 
...
I also used the GBIF occurrence data to be able to graphically analyze the geographic patterns of turtles. Due to the size of this [dataframe](https://github.com/SamWololo/Threatened-Species/blob/master/2.data_download_and_processing.R), I added it to my gitignore. The dataframe contained 348,778 observations with 184 variables for 273 species of turtles. I used pipes here as well, and thus loaded tidyverse. 

```{r message=FALSE, warning=FALSE}
gbif_records<-read.csv("Data/Base/gbif_records2.csv")

```
I then cleaned many of the variables, being that many in the data frame were beyond the scope of this project. 
```{r echo=TRUE}
gbif_records<-
  gbif_records %>% 
...
```
This yielded a data frame still with 204,747 occurrences but with only 4 variables. These variables are:
```{r echo=TRUE}
names(gbif_records)

```
After this, I removed the duplicate occurences in the data frame, yielding 82,837 observations for 4 variables for 273 species of turtles. 
```{r}
...
### Merging
At this stage the data has been cleaned for the purposes of this project. The next step is combining the data into one data frame for it to be usable during analysis. I then wrote this file into a .csv. 
...
### Data analysis
In order to demonstrate the relationship between the body mass (g) life history trair to the IUCN category, we drew a box and whisker plot. 
```{r message=FALSE, warning=FALSE}
All_Dataframes_df %>% 
  group_by(Binomial) %>% 
